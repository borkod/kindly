...
Examples:
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		for _, n := range args {
			...
			x, yc, err := k.Check(ctx, viper.GetString("source2"), n)
			if err != nil {
			...
			if !viper.GetBool("output") && x {
				log.Println("Package: ", n, string("\u001b[32m"), "OK", string("\u001b[0m"))
			}
			if !viper.GetBool("output") && !x {
				log.Println("Package: ", n, string("\u001b[31m"), "NO", string("\u001b[0m"))
			}
...
func init() {
	...
	checkCmd.Flags().StringP("source2", "a", "", "Package source name.")
